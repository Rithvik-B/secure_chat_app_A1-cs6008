...
bool drain_records(Conn& c)
{
    ...
    for (;;) {
        ...
        if (type == proto::REC_HANDSHAKE) {
            ...
            std::string err;
            auto st = c.hs.feed(body.data(), body.size(), reply, err);
            ...
            if (!reply.empty())
                enqueue(c, proto::REC_HANDSHAKE, reply.data(), reply.size());
...
int main(int argc, char** argv)
{
    ...
    g_log.log("START phase2 DH + AES-256-GCM relay on " + bind_addr + ":" +
