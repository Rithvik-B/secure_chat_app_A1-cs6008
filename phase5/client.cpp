// Chat client: authenticated client-server channel (Phase 3), end-to-end
// encryption between clients (Phase 4, /e2e), and periodic rekeying for forward
// secrecy (Phase 5). Three threads -- stdin, socket reader, rotation timer --
// with sends serialised. Outgoing messages are echoed locally.
...
void display(const std::string& line)
{
    ...
    if (verb == "FROM") {
        ...
        if (has(e2e::TAG_INIT)) {
            std::string ack, note, err;
            bool ok;
            { std::lock_guard<std::mutex> lk(g_e2e_mu);
              ok = g_e2e.on_init(g_user, sender, text.substr(std::strlen(e2e::TAG_INIT)), ack, note, err); }
            if (!ok) { say("!! E2E request from " + sender + " failed: " + err); return; }
            if (!ack.empty() && !send_line("MSG " + sender + " " + ack)) bail(1);
            if (!note.empty()) say(note);          // "established" or "rekey -> epoch N"
        } else if (has(e2e::TAG_ACK)) {
            std::string note, err;
            bool ok;
            { std::lock_guard<std::mutex> lk(g_e2e_mu);
              ok = g_e2e.on_ack(g_user, sender, text.substr(std::strlen(e2e::TAG_ACK)), note, err); }
...
// Drives key rotation: once a second, asks the E2E manager for any rotations
// that are due and relays the INITs it produces. The rekey itself completes
// when the ACK arrives (in the reader thread).
void rotation_timer()
{
    for (;;) {
        std::this_thread::sleep_for(std::chrono::seconds(1));
        std::vector<e2e::Action> actions;
        std::vector<std::string> notes;
        { std::lock_guard<std::mutex> lk(g_e2e_mu); g_e2e.tick(g_user, actions, notes); }
        for (const auto& a : actions)
            if (!send_line("MSG " + a.peer + " " + a.payload)) bail(1);
    }
}
...
int main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--rekey") {   // override the 60 s rotation interval (for demos/tests)
            if (i + 1 >= argc) { usage(argv[0]); return 2; }
            int n = std::stoi(argv[++i]);
            if (n > 0) { e2e::REKEY_SEC = n; e2e::FALLBACK_SEC = n + 15; }
        }
    ...
    std::thread(rotation_timer).detach();
