// Relay server: single-threaded poll() loop, per-connection Diffie-Hellman and
// AES-256-GCM. Each client negotiates its own key; routing is unchanged from
// Phase 1, the server just decrypts inbound records and re-encrypts outbound.
...
pki::ServerCreds                     g_creds;      // certificate + private key

// Queues a record; the poll loop writes it when the socket is writable.
void enqueue(Conn& c, uint8_t type, const uint8_t* payload, size_t len)
{
    if (len + 1 > proto::MAX_RECORD) return;

    uint32_t be = htonl(static_cast<uint32_t>(len + 1));
...
// Sealed under the connection's key: nothing reaches the socket in the clear.
void send_line(Conn& c, const std::string& line)
{
    if (!c.sess.established()) { c.want_close = true; return; }
    std::vector<uint8_t> ct;
    if (!c.sess.seal(line, ct)) { c.want_close = true; return; }
    enqueue(c, proto::REC_APPDATA, ct.data(), ct.size());
}

// Alerts are plaintext: sent only while tearing down, often before any key.
void send_alert(Conn& c, const std::string& reason)
{
    enqueue(c, proto::REC_ALERT,
...
bool drain_records(Conn& c)
{
    ...
    for (;;) {
        ...
        if (type == proto::REC_HANDSHAKE) {
            if (c.sess.established()) {
                g_log.log("DROP  " + c.label() + " : handshake record after completion");
                return false;
            }
            std::vector<std::vector<uint8_t>> replies;
            std::string err;
            auto st = c.hs.feed(body.data(), body.size(), replies, err);
            if (st == sec::ServerHandshake::Status::Failed) {
                g_log.log("DROP  " + c.label() + " : handshake failed: " + err);
                send_alert(c, err);
                return true;                       // flush the alert, then close
            }
            for (const auto& r : replies)
                enqueue(c, proto::REC_HANDSHAKE, r.data(), r.size());
            if (st == sec::ServerHandshake::Status::Done) {
                c.sess.install(c.hs.keys(), sec::Role::Server);
                g_log.log("KEX   " + c.peer + " session established, fingerprint=" +
                          c.sess.fingerprint());
            }
            continue;
        }
        ...
        if (!c.sess.established()) {
            g_log.log("DROP  " + c.label() + " : application data before handshake");
            return false;
        }
        if (body.size() > proto::MAX_PLAINTEXT + crypto::TAG_LEN) {
            g_log.log("DROP  " + c.label() + " : payload too large");
            return false;
        }

        // Tag failure -> reject, never process: a flipped byte is a hard error.
        std::string line;
        if (!c.sess.open(body.data(), body.size(), line)) {
            g_log.log("DROP  " + c.label() +
                      " : AES-GCM authentication failed, record rejected");
            return false;
        }
...
void accept_new(int listen_fd)
{
    for (;;) {
        ...
        c.hs.set_credentials(g_creds.cert_der, g_creds.key);
...
void usage(const char* argv0)
{
    std::cerr << "usage: " << argv0
              << " --cert FILE --key FILE [--bind ADDR] [--port N] [--log FILE]\n";
}
...
int main(int argc, char** argv)
{
    ...
    std::string cerr_msg;
    if (!pki::load_server_creds(cert_path, key_path, g_creds, cerr_msg)) {
        std::cerr << "error: " << cerr_msg << "\n";
        return 1;
    }
    ...
    g_log.log("START phase3 DH + AES-256-GCM relay, authenticated (PKI) on " + bind_addr + ":" +
