...
int main()
{
    ...
    std::printf("end-to-end session\n");
    {
        ...
        std::string init, ack, err;
        bool ok = alice.start("alice", "bob", init, err);
        ok = ok && bob.on_init("bob", "alice", init.substr(std::strlen(e2e::TAG_INIT)), ack, err);
        ok = ok && alice.on_ack("alice", "bob", ack.substr(std::strlen(e2e::TAG_ACK)), err);
        ...
        // A third party with a different secret cannot read the traffic.
        e2e::Manager eve;
        std::string ei;
        eve.start("eve", "alice", ei, err);
        // eve never completed a handshake with alice's actual key, so opening fails.
