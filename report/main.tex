...
\graphicspath{{../evidence/phase1/}{../evidence/phase2/}{../evidence/phase3/}{../evidence/phase0/}}
...
\fancyhead[R]{\footnotesize\sffamily\color{accent}Phases 0--3}
...
{\sffamily\large Phase 0 --- Deployment Environment\\[2pt]
 ...
 Phase 3 --- Server Authentication via PKI}\\[10pt]
...
and server authentication via a PKI, each with its verification.
...
the server and the same attack now fails. No TLS library is used anywhere.
\end{minipage}
...
man-in-the-middle attack against it (Phase~2), and server authentication via a PKI that defeats that
attack (Phase~3), each with its verification.
...
./client 10.10.0.10 alice --ca certs/ca-cert.pem --expect chatserver.local  # phase 3
...
Phases 2 and 3 secure the client--server link, but the server itself still reads every message it
relays. Phase~4 adds end-to-end encryption between the two clients: they establish a shared key the
server cannot derive, so it relays only opaque ciphertext. Phase~5 then rotates that key periodically
for forward secrecy.
