...
\graphicspath{{../evidence/phase1/}{../evidence/phase0/}}
...
\fancyhead[R]{\footnotesize\sffamily\color{accent}Phases 0--1}
...
{\sffamily\large Phase 0 --- Deployment Environment\\[2pt]
 Phase 1 --- Baseline Chat Application}\\[10pt]
...
the deployment environment and the baseline plaintext application together with its verification.

\medskip
\noindent
Four virtual machines were provisioned on an isolated virtual network and connectivity verified
before any application code was run. The application is written in C++17 over POSIX sockets, with a
length-prefixed record protocol chosen so that the framing remains valid once message bodies stop
being printable text. Phase~1 deliberately provides no confidentiality: the server logs the full
content of every message it relays, and a packet capture taken on the network segment shows the
entire conversation in cleartext. That is the baseline the remaining phases must dismantle.
\end{minipage}
...
This report documents the first stage of a one-to-one chat application built over TCP sockets and
progressively hardened across five phases. It covers the deployment environment (Phase~0) and the
baseline plaintext chat application together with its verification (Phase~1).
...
\begin{lstlisting}
cd phase1 && make            # builds ./server and ./client
...
Phase~2 adds confidentiality between each client and the server. The record framing, application
grammar and routing logic described here are expected to carry over unchanged, with the work confined
to the record body. The captures above will be repeated on the new protocol so that the two can be
compared directly at the same offsets.
