...
\graphicspath{{../evidence/phase1/}{../evidence/phase2/}{../evidence/phase3/}{../evidence/phase4/}{../evidence/phase0/}}
...
\fancyhead[R]{\footnotesize\sffamily\color{accent}Phases 0--4}
...
{\sffamily\large Phase 0 --- Deployment Environment\\[2pt]
 ...
 Phase 4 --- End-to-End Encryption Between Clients}\\[10pt]
...
server authentication via a PKI, and end-to-end encryption between the two clients, each with its
verification.
...
with no change to the server at all. No TLS library is used anywhere.
\end{minipage}
...
attack (Phase~3), and end-to-end encryption between the clients (Phase~4), each with its
verification.
...
#   phase 4: type "/e2e bob" in the client to start end-to-end with bob
...
Phase~4 keeps one end-to-end key for the whole session, so an attacker who later compromised that key
could read every message ever exchanged under it. Phase~5 adds forward secrecy: the C1$\leftrightarrow$C2
key is renegotiated on a timer, each new key independent of the last and the old one discarded, so a
compromised key exposes only a bounded window of traffic.
